...
import 'package:efood_multivendor/data/model/BeeResponse/bee_response_model.dart';
import 'package:efood_multivendor/data/model/BeeResponse/Register_response_model.dart';
...
import '../data/model/BeeResponse/forgetresponsemodel/Forget_response_model.dart';
import '../data/model/BeeResponse/verifyModel/Verify_response_model.dart';
...
class AuthController extends GetxController implements GetxService {
  ...
  Future<RegisterResponseModel> registration(SignUpBody signUpBody) async {
    _isLoading = true;
    update();
    Response response = await authRepo.registration(signUpBody);
    RegisterResponseModel responseModel;
    if (response.statusCode == response.body['statusCode']) {
      responseModel=RegisterResponseModel.fromJson(response.body);
      ...
      responseModel = RegisterResponseModel( statusCode:response.statusCode, message:response.statusText);
    }
    _isLoading = false;
    update();
    return responseModel;
  ...
  Future<BeeResponseModel> login(String contry_code, String phone, String password,String device_token,String device_type) async {
    _isLoading = true;
    update();

    Response response = await authRepo.login(contry_code: contry_code, phone: phone, password: password,device_token: device_token,device_type: device_type);


    BeeResponseModel responseModel;
    if (response.statusCode == response.body['statusCode']) {

      //Map<String, dynamic> decodedData =json.decode(response.body);
      responseModel=BeeResponseModel.fromJson(response.body);
      ...
      responseModel = BeeResponseModel(statusCode: response.statusCode, message:response.statusText);
    }
    _isLoading = false;
    update();
    return responseModel;
  ...
  Future<ForgetResponseModel> forgetPassword(String email) async {
    _isLoading = true;
    update();
    Response response = await authRepo.forgetPassword(email);

    ForgetResponseModel responseModel;
    if (response.statusCode == response.body['statusCode']) {
      responseModel=ForgetResponseModel.fromJson(response.body);

    } else {
      responseModel = ForgetResponseModel(statusCode:response.statusCode  ,message:response.statusText);
    }
    _isLoading = false;
    update();
    return responseModel;
  ...
  Future<VerifyResponseModel> verifyToken(String email) async {
    _isLoading = true;
    update();
    Response response = await authRepo.verifyToken(email, _verificationCode);
    VerifyResponseModel responseModel;
    if (response.statusCode == 200) {
      responseModel = VerifyResponseModel.fromJson(response.body);
    } else {
      responseModel = VerifyResponseModel(statusCode:response.statusCode, message:response.statusText);
    }
    _isLoading = false;
    update();
    return responseModel;
  ...
  Future<BeeResponseModel> resetPassword(String number, String password) async {
    _isLoading = true;
    update();
    Response response = await authRepo.resetPassword(number, password);
    BeeResponseModel responseModel;
    if (response.statusCode == response.body['statusCode']) {
      responseModel = BeeResponseModel(statusCode: response.body['statusCode'],message: response.body['message']);
    } else {
      responseModel = BeeResponseModel(statusCode: response.statusCode,message: response.statusText);
    }
    _isLoading = false;
  ...
  Future<VerifyResponseModel> verifyPhone(String phone, String otp) async {
    _isLoading = true;
    update();
